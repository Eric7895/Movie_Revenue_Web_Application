...
Sen Sharma et al. (2021) provided foundational research for predicting box office success using pre-release metadata. They introduced a large-scale dataset and used various machine learning models—including logistic regression, random forests, and neural networks—to classify movies into revenue brackets. Their study emphasized the importance of features like budget, star power, genre, and release month, and proposed a novel method to quantify star power using historical success and IMDb metrics. Moreover, Madongo's research is done over a hybrid deep learning model that combines visual features from movie posters with metadata for improved prediction (Madongo, 2023). Their corresponding accuracy metrics are 60% (Sharma) and 59.3% (Madongo)

We wanted to build on this foundation to build a project that combines the most successful elements of these other projects to create an optimal solution that would lead to better results. Importantly, it also includes actor and director scoring mechanisms based on historical box office performance, introducing domain-specific enhancements to the feature set, like audience engagement metrics (trailer view/likes). These design choices informed our own approach, particularly in how we engineered similar role-based features to reflect past project success and career consistency, such as star score.
...
Additionally, we attempted to scrape actor, director, and writer "billboards" for feature engineering purposes. We ensured that the sources used were recently updated and broadly representative. For directors and writers, we used an IMDb list created by user “ur1612690”^[https://www.imdb.com/list/ls026411399], which was most recently updated on April 15, 2025, and has received 98,000 views. For actors, we used The Numbers billboard^[Launched by Bruce Nash on October 17th, 1997, now became one of the largest freely-available database of movie business information on the Web] to collect actor information and corresponding star scores. We acknowledge that these billboards—especially those created by users—may contain biases, and addressing this bias is a potential area for future improvement.

Moving on to data processing, we removed all movies with null values and still retained a relatively sufficient number of records for model training. We also eliminated duplicate entries by using a composite identifier of movie title and release date. After cleaning the data, we began feature engineering. Our encoding approaches for each categorical variable are as follows:

-   **Director and Writer**: We counted the total number of famous directors and writers associated with each movie, based on their appearance in a well known billboard lists (98k views).
...
-   **Release Date**: We created two binary variables—is_holiday_release and is_competitive_month. The holiday variable flags releases near major holidays, while the competitive month variable flags historically high-grossing periods (May, June, July, November, and December).

-   **Production Companies**: We created a binary variable, production_popular, to indicate whether a movie was produced by a major company. Our list includes: Warner Bros, Universal Pictures, Walt Disney, Columbia Pictures, 20th Century Fox, etc. In the future, we may use string similarity measures (e.g., cosine similarity) to better capture variations in company names.
...
-   **Keywords**: We used the pretrained BERT^[BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding] model 'all-mpnet-base-v2'^[https://huggingface.co/sentence-transformers/all-mpnet-base-v2] to embed each keyword into high-dimensional vectors. We then applied Principal Component Analysis (PCA) to reduce the dimensionality, selecting the first principal component. We ensured that the first eigenvalue explained at least 40% of the variance. Using a model like BERT helps preserve the semantic meaning of keywords as movies that have similar keywords will be closed together in the high dimensional embedded space. 

- **Star Score**: We decided to aggregate cumulative star score from the numbers website, individual star score  represent points assigned to each of the leading stars of top 100 (based on domestic box office) movies in the current year and two preceding years. For appearing in the number one movie in a year a star gets 100 points, the number two movie 99 points and so on. This rewards actors for appearing in a number of hit films over the course of three years more than starring in just one monster hit over the same time period.
...
-   **Ordinary Least Squares (OLS)**: Ordinary Least Squares (OLS) is the foundational linear regression technique^[https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LinearRegression.html]. It fits a hyperplane that minimizes the sum of squared residuals (differences between observed and predicted values). OLS is prioritize for its simplicity and interpretability—each coefficient represents the expected change in the target (revenue) for a one‑unit change in the predictor, holding others constant. However, OLS assumes:

-    **Elastic Net Regression**: Combination of L1 and L2 regularization, effectively handling multicollinearity^[https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.ElasticNet.html].

### Nonlinear Models

Advanced nonlinear techniques included:

-   **Generalized Additive Models (GAM)**: Extended linear modeling using spline functions for capturing nonlinear effects^[https://pygam.readthedocs.io/en/latest/].
-   **Random Forest Regressor (RFR)**: Random Forest constructs an ensemble of decision trees on bootstrapped samples, each using random subsets of features at splits. By averaging predictions across many trees, it reduces variance and prevents overfitting. Its non‑parametric nature allows it to automatically capture complex nonlinearities and interactions among predictors without explicit feature engineering. Moreover, it provides feature importance scores, helping identify the most influential variables^[https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html].
-   **XGBoost**: XGBoost (Extreme Gradient Boosting) builds trees sequentially, where each new tree corrects errors from the ensemble so far. It uses second‑order gradient information and built‑in regularization (L1 and L2) to enhance predictive power while mitigating overfitting. XGBoost is optimized for speed and can handle large datasets efficiently, making it a standard for structured data tasks requiring high accuracy^[https://xgboost.readthedocs.io/en/stable/python/python_intro.html].
...
```{r result_table}
result <- data.frame (
  Model = c('OLS', 'Elastic Net', 'GAM', 'RFR', 'XGBoost'),
  RMSE = c(10.02, 10.17, 9.4, 10.25, 9.79),
  R2 = c(0.8303, 0.8253, 0.8508, 0.8225, 0.8438),
  Training_Time_With_Hyperparameter_Tuning = c('700 ms','10 s','1.1 s','1 m 55 s
','2.9 s')
)
...
result %>%
  kable(
    ...
    booktabs = TRUE,
    align = c('l', rep('c', 5)),
    col.names = c('Model', 'RMSE', 'R2', 'Training time (With Hyperparameter Tuning)')
  ) %>%
...
## Model Specific Detail/Diagnostic

### OLS

```{r, fig.pos="H", echo=FALSE}
knitr::include_graphics("diagnostic_ols.png")
```
...
#### Key Nonlinear Effects based on the GAM graph:
...
```{r, fig.pos="H", echo=FALSE}
knitr::include_graphics("rfr_importance.png")
```

-   Polynomial interactions does not improved model performance.

-   Feature importance analysis strongly highlights PCA-derived trailer metrics
as crucial predictors.

### XGboost

```{r, fig.pos="H", echo=FALSE}
knitr::include_graphics("xg_importance.png")
```

- Audience Engagement(above 40%): Trailer likes is the strongest importance predictor, far outweighing traditional metrices. 

- Temporal Factors: Release year has major effect, reflecting market evolution over time

- Genre Preference: Adventure, Action, and Horror movies showed a higher significance compared to other genres. 

#### Implications

- Marketing strategies should prioritize trailer engagement 

- Revenue forecasts need to account for yearly trend

- Traditional factors like budget has limit impact 
...
By having a database connection, we implemented website functionality using the FastAPI framework. The FastAPI Python module allows us to quickly set up a basic functioning website that supports Create, Read, Update, and Delete (CRUD) endpoints. These operations allows the user to effectively communicate with the data source and update/correct incorrect information without having to scroll through the csv file. 
...
Across all methods—from OLS and GAM to Random Forest and XGBoost—we observed that greater model flexibility consistently improved predictive accuracy. We find that GAM strikes the best balance between performance (R-squared = 0.8504) and complexity (four terms) because it captures nuanced, nonlinear relationships, such as diminishing returns for mid-range budgets and the synergy between budget and star power by fitting each of the covariates to the response variable using a smooth function and take the additive sum of the partial contribution.
...
-   Outlier Management: Compute Cook’s distance^[a statistical measure that quantifies the influence of individual data points on a regression model. It helps identify data points that, if removed, would significantly alter the model's predictions or parameter estimates] to identify influential observations; apply Winsorization or conditional trimming to mitigate distortion.
...
-   Feature Expansion: Integrate social‑media sentiment and search‑trend data, as well as new interaction terms (e.g., trailer engagement × release month) into both linear and tree‑based models.
...
- Zhang, Y., Luo, X., & Yang, Y. (2009). Predicting movie success with machine learning techniques. International Journal of Computer Applications, 1(1), 1–6.​
