...
result <- data.frame (
  ...
  RMSE = c(10.33, 10.55, 10.07, 10.28, 9.79),
  R2 = c(0.8153, 0.8075, 0.8244, 0.8171, 0.8438),
  Training_Time_With_Hyperparameter_Tuning = c('500 ms','6 s','1 s','1 min','1 s')
)
...
Across all methods—from OLS and GAM to Random Forest and XGBoost—we observed that greater model flexibility consistently improved predictive accuracy. We find that GAM strikes the best balance between performance (R-squared = 0.8244) and complexity (four terms) because it captures nuanced, nonlinear relationships, such as diminishing returns for mid-range budgets and the synergy between budget and star power.
